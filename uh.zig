...
pub fn main() !u8 {
    ...
    for (headers) |header, header_index| {
...
fn parseJson(json_filename: []const u8) ![]SymbolDef {
    ...
    const tree = blk: {
        var parser = json.Parser.init(arena.allocator(), false); // false is copy_strings
        defer parser.deinit();
    ...
    while (root_it.next()) |pair| {
        ...
        const headers = blk: {
            ...
            break :blk headers.toOwnedSlice(global.arena.allocator());
        };
        ...
        symbols.append(global.arena.allocator(), SymbolDef{
            .id = id,
            .defs = defs.toOwnedSlice(global.arena.allocator()),
        }) catch |e| oom(e);
    }
